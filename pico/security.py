...
_BLOCKED_COMMAND_PATTERNS = (
    (re.compile(r"\brm\s+-[^\n]*r[^\n]*f", re.I), "recursive_force_delete"),
    (re.compile(r"\b(?:del|rmdir)\b[^\n]*(?:/s|/q)", re.I), "windows_recursive_delete"),
    (re.compile(r"\bformat(?:\.com)?\b", re.I), "disk_format"),
    (re.compile(r"\b(?:shutdown|reboot|diskpart)\b", re.I), "system_control"),
    (re.compile(r"\bgit\s+(?:reset\s+--hard|clean\s+-[^\n]*f|restore\s+--source)", re.I), "destructive_git"),
    (re.compile(r"\bremove-item\b[^\n]*-recurse[^\n]*-force", re.I), "powershell_recursive_delete"),
)
_HIGH_RISK_COMMAND_PATTERNS = (
    (re.compile(r"\bsudo\b|\brunas\b", re.I), "privileged_command"),
    (re.compile(r"\bgit\s+push\b|\bgit\s+reset\b|\bgit\s+clean\b", re.I), "repository_mutation"),
    (re.compile(r"\b(?:pip|uv|npm|pnpm|yarn)\s+(?:install|add|remove)\b", re.I), "dependency_mutation"),
)


def classify_shell_command(command):
    """Classify a command before it reaches a shell.

    This is intentionally a conservative deny-list for obviously destructive
    commands plus a small high-risk signal.  It is an additional guardrail,
    not a replacement for approval, workspace isolation, or OS sandboxing.
    """
    ...
    for pattern, reason in _HIGH_RISK_COMMAND_PATTERNS:
        if pattern.search(command):
            return {"decision": "allow", "risk_level": "high", "reason": reason}
    return {"decision": "allow", "risk_level": "low", "reason": "command_pattern_not_flagged"}
