...
SCRIPTED_MODEL_OUTPUTS = {
    ...
    "plan_verify_status": [
        '<plan>{"goal":"Update and verify the fixture status","tasks":[{"id":"update_status","title":"Update README status","description":"Change the pending status marker to verified."}]}</plan>',
        '<tool name="patch_file" path="README.md"><old_text>status: pending</old_text><new_text>status: verified</new_text></tool>',
        "<final>README status verified.</final>",
    ],
    "verification_retry": [
        '<tool name="patch_file" path="README.md"><old_text>status: pending</old_text><new_text>status: pending</new_text></tool>',
        "<final>First attempt finished.</final>",
        '<tool name="patch_file" path="README.md"><old_text>status: pending</old_text><new_text>status: passed</new_text></tool>',
        "<final>README status passed after verification feedback.</final>",
    ],
    "plan_replay_evidence": [
        '<plan>{"goal":"Inspect the fixture and preserve evidence","tasks":[{"id":"inspect_fixture","title":"Inspect README fixture","description":"Read README.md and record a replayable run."}]}</plan>',
        '<tool>{"name":"read_file","args":{"path":"README.md","start":1,"end":20}}</tool>',
        "<final>README inspected and evidence recorded.</final>",
    ],
}
...
def _current_locale():
    # Benchmark files are encoded explicitly as UTF-8.  Keep this metadata
    # stable across Windows locale names (for example ``Chinese...936``) and
    # POSIX ``C.UTF-8`` so artifacts can be compared between machines.
    return "C.UTF-8"
...
def _now_in_timezone(timezone_name):
    try:
        ...
        # The benchmark has no runtime dependency on the optional tzdata
        # package.  Keep Asia/Shanghai reproducible when a minimal Python
        # installation does not ship the IANA database.
        if str(timezone_name) in {"Asia/Shanghai", "Asia/Chongqing", "PRC"}:
...
def validate_benchmark(data, repo_root=None):
    ...
    for index, task in enumerate(tasks):
        ...
        # Benchmark schemas may exercise experimental capabilities explicitly,
        # while the normal V1 runtime keeps them disabled by default.
        valid_tools = legal_tool_names(include_experimental=True)
...
def render_benchmark_markdown(artifact):
    ...
    lines = [
        ...
        f"- Model: `{artifact.get('reproducibility', {}).get('model_name', '')}` / `"
        f"{artifact.get('reproducibility', {}).get('model_version', '')}`",
        ...
        "## Task Results",
        "",
        "| Task | Category | Status | Tools | Verification | Failure |",
        "| --- | --- | --- | ---: | --- | --- |",
    ]
    for row in artifact.get("rows", []):
        verification = dict(row.get("verification", {}) or {})
        verification_status = verification.get("status") or (
            "passed" if row.get("verifier_passed") else "failed"
        )
        lines.append(
            "| {id} | {category} | {status} | {tools} | {verification} | {failure} |".format(
                id=row.get("id", ""),
                category=row.get("category", ""),
                status=row.get("status", ""),
                tools=row.get("tool_steps", ""),
    ...
    lines.extend(
        [
            ...
            "## Reproducibility",
            "",
            f"- Fixture snapshot: `{artifact.get('reproducibility', {}).get('fixture_snapshot_id', '')}`",
            f"- Temperature: `{artifact.get('reproducibility', {}).get('decoding', {}).get('temperature', '')}`",
...
class BenchmarkEvaluator:
    ...
    def run_task(self, task):
        ...
        agent = Pico(
            ...
            max_new_tokens=self.max_new_tokens,
            allowed_tools=task["allowed_tools"],
            enable_delegate="delegate" in task["allowed_tools"],
