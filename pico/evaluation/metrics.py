...
def _is_tool_completion_event(event):
    """Accept current and legacy completion event names when aggregating runs."""

    return (
        event.get("event") in {"tool_finished", "tool_completed", "tool_executed"}
...
def _security_agent(workspace_root, approval_policy="auto", read_only=False, enable_delegate=False):
    workspace = WorkspaceContext.build(workspace_root)
    store = SessionStore(workspace_root / ".pico" / "sessions")
    return Pico(
        model_client=FakeModelClient([]),
...
def _scenario_empty_delegate_task(workspace_root):
    agent = _security_agent(workspace_root, enable_delegate=True)
...
def _followup_trace_metrics(agent):
    ...
    events = [json.loads(line) for line in trace_path.read_text(encoding="utf-8").splitlines() if line.strip()]
    repeated_reads = sum(1 for event in events if _is_tool_completion_event(event) and event.get("name") == "read_file")
