...
def _clip_prefix(text, limit):
    """Clip the stable prefix without dropping resume-critical control state."""

    text = str(text)
    if len(text) <= limit:
        return text
    markers = [position for marker in ("Task checkpoint:", "Plan:") if (position := text.find(marker)) >= 0]
    if not markers:
        return _tail_clip(text, limit)
    marker = min(markers)
    suffix = text[marker:]
    separator = "\n...[prefix truncated]...\n"
    if len(suffix) + len(separator) >= limit:
        return suffix[: max(0, limit - 3)] + "..."
    head_budget = limit - len(separator) - len(suffix)
    return text[:head_budget] + separator + suffix
...
class ContextManager:
    ...
    def build(self, user_message):
        ...
        skills_enabled = True
        if hasattr(self.agent, "feature_enabled"):
            skills_enabled = self.agent.feature_enabled("skills")
        if skills_enabled and skill_registry is not None:
            selected_skills = skill_registry.select(
                user_message,
                available_tools=getattr(self.agent, "tools", {}),
                limit=3,
            )
            rendered_skills = skill_registry.render(selected_skills)
            if rendered_skills:
        ...
        plan_text = ""
        if hasattr(self.agent, "render_plan_text"):
            plan_text = str(self.agent.render_plan_text() or "").strip()
        if plan_text:
            section_texts["prefix"] = section_texts["prefix"] + "\n\n" + plan_text
        selected_notes = []
    ...
    def _render_sections(self, section_texts, budgets, selected_notes=None):
        ...
        for section in SECTION_ORDER:
            ...
            if section == CURRENT_REQUEST_SECTION:
                ...
                raw = section_texts[section]
                clipper = _clip_prefix if section == "prefix" else _tail_clip
                rendered_text = clipper(raw, int(budget)) if budget is not None else raw
    ...
    def _metadata(self, prompt, rendered, budgets, reduction_log, selected_notes, user_message, section_texts, selected_skills=None):
        ...
        return {
            ...
            "skills": {
                ...
                "selected_paths": [str(skill.path) for skill in selected_skills],
            },
        }
