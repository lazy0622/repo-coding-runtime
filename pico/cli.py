...
from .mcp import MCPToolProvider, load_mcp_server_configs
from .providers.clients import AnthropicCompatibleModelClient, OllamaModelClient, OpenAICompatibleModelClient
from .replay import render_replay, resolve_trace_path
...
def build_agent(args):
    ...
    workspace_cwd = args.cwd
    if getattr(args, "workspace_mode", "direct") == "worktree":
        workspace_lease = WorkspaceLease.create(args.cwd, base_dir=getattr(args, "worktree_base", None))
        workspace_cwd = workspace_lease.workspace_root
    workspace = WorkspaceContext.build(workspace_cwd)
    load_project_env(workspace.repo_root)
    configured_secret_names = _configured_secret_names(args)
    store = SessionStore(workspace.repo_root + "/.pico/sessions")
    model = _build_model_client(args)
    skill_paths = tuple(getattr(args, "skill_paths", []) or ()) or None
    mcp_config = getattr(args, "mcp_config", None)
    if not mcp_config:
        default_mcp_config = Path(workspace.repo_root) / ".pico" / "mcp.json"
        mcp_config = str(default_mcp_config) if default_mcp_config.exists() else None
    tool_providers = []
    if mcp_config:
    ...
    if session_id:
        return Pico.from_session(
            ...
            enable_delegate=getattr(args, "enable_delegate", False),
            enable_subagents=getattr(args, "enable_subagents", False),
            auto_promote_memory=getattr(args, "auto_promote_memory", False),
            plan_mode=getattr(args, "plan_mode", True),
...
def build_arg_parser():
    ...
    parser.add_argument("--replay", default=None, help="Replay a trace.jsonl file, run directory, or run id and exit.")
    parser.add_argument(
        "--enable-delegate",
        action="store_true",
        help="Enable the experimental read-only delegate tool; disabled by default in V1.",
    )
    parser.add_argument(
        "--enable-subagents",
        "--v2",
        ...
        action="store_true",
        help="Enable the V2 bounded read-only task-graph supervisor.",
    )
    parser.add_argument(
        "--auto-promote-memory",
        action="store_true",
        help="Opt in to heuristic promotion of explicitly requested durable memory.",
    )
    parser.add_argument(
        "--skill-path",
        dest="skill_paths",
        action="append",
        default=[],
        help="Directory or SKILL.md path to discover. Can be repeated.",
    )
    parser.add_argument("--mcp-config", default=None, help="JSON file containing an mcpServers object.")
...
def main(argv=None):
    ...
    if args.replay:
        trace_path = resolve_trace_path(args.replay, cwd=args.cwd)
        print(render_replay(trace_path, include_json=True))
        return 0
    ...
    while True:
        ...
        if user_input == "/memory":
            ...
            continue
        if user_input == "/plan":
            print(agent.render_plan_text() or "No active plan")
            continue
