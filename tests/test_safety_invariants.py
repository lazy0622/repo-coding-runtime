...
def test_destructive_shell_command_is_blocked_before_execution(tmp_path):
    agent = build_agent(tmp_path, [])

    result = agent.run_tool("run_shell", {"command": "git reset --hard HEAD", "timeout": 20})

    assert "command blocked by safety policy" in result
    assert agent._last_tool_result_metadata["tool_error_code"] == "command_blocked"
    assert agent._last_tool_result_metadata["security_event_type"] == "command_blocked"
...
def test_symlink_path_traversal_is_rejected(tmp_path):
    ...
    try:
        ...
        if os.name == "nt" and getattr(exc, "winerror", None) == 1314:
            pytest.skip("Windows symlink creation requires the SeCreateSymbolicLink privilege")
        raise
    agent = build_agent(tmp_path, [])
