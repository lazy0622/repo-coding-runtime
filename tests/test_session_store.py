...
def test_session_store_atomic_save_leaves_no_temporary_file(tmp_path):
    store = SessionStore(tmp_path / ".pico" / "sessions")

    path = store.save({"id": "atomic", "history": []})

    assert json.loads(path.read_text(encoding="utf-8"))["id"] == "atomic"
