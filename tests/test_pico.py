...
def test_v1_experimental_delegate_is_disabled_by_default(tmp_path):
    ...
    assert "- delegate(" not in agent.prompt("Inspect the repository")
    assert "prompt_cache" not in agent.feature_flags


def test_read_only_tool_does_not_create_tool_checkpoint(tmp_path):
    agent = build_agent(
        tmp_path,
        [
    ...
    assert agent.ask("Read the README") == "Done."
    ...
    assert all(event["trigger"] != "tool_finished" for event in checkpoint_events)
    assert checkpoint_events[-1]["trigger"] == "run_finished"


def test_durable_memory_promotion_requires_explicit_opt_in(tmp_path):
    agent = build_agent(
        tmp_path,
        ["<final>Project convention: Keep the runtime small.</final>"],
    )

    agent.ask("Capture this stable fact into durable memory.")

    assert not (tmp_path / ".pico" / "memory" / "topics" / "project-conventions.md").exists()
...
def test_successful_run_persists_run_artifacts_and_stop_reason(tmp_path):
    ...
    assert "tool_finished" in trace_events


def test_trace_and_report_redact_secret_env_values(tmp_path):
    secret = "sk-test-secret-123"
    # Keep the host shell runtime variables on Windows; this test only needs
    # to override the secret value, not remove %ComSpec%/%SystemRoot%.
    ...
    tool_events = [event for event in trace_events if event["event"] == "tool_finished"]
    assert tool_events
...
def test_write_file_trace_records_minimum_tool_contract_fields(tmp_path):
    ...
    tool_event = [event for event in trace_events if event["event"] == "tool_finished"][-1]
...
def test_reviewer_skeleton_docs_exist():
    ...
    assert "Repo Coding Runtime Architecture" in architecture_text
